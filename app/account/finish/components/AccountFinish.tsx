...
export default function AccountFinish({ session }: Props) {
  const [textClass, setTextClass] = useState<string>(
    'text-xl inset-0 text-white  grid place-items-center bg-black/50 backdrop-blur-sm z-[999]'
  );
  ...
  return (
    <div
      className={`${montserrat.variable} flex flex-col justify-center items-center p-5 min-h-screen dark:text-white`}
    >
      ...
      <h1 className="text-3xl font-montserrat font-bold p-5">
        THE <span className="text-fieldGreen">F</span>
        IELD
      </h1>
      <h1>
        Hey there, <span className="font-bold">{session.user?.username}</span>{' '}
        👋
      </h1>
