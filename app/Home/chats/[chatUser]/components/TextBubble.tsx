...
export default function TextBubble({
  content, date, type, nextType, myImage,
}:Props) {
    ...
    return (
      <div className="flex w-full mt-2 space-x-3 justify-end">
        <div>
          ...
          <span className={`flex text-xs text-zinc-400 leading-none justify-end pt-1 ${nextType === 'sent' && 'hidden'}`}>{date}</span>
        </div>
  ...
  return (
    <div className="flex w-full mt-2 space-x-3">
      <div className={`h-10 w-10  ${nextType === 'received' && 'dark:bg-black'}`}>
        <CustomAvatar size={nextType === 'received' ? 15 : 10} imgUrl={imageLink} />
      </div>
      <div>
        ...
        <span className={`text-xs text-zinc-400 leading-none ${nextType === 'received' && 'hidden'}`}>{date}</span>
      </div>
    </div>
  );
}
